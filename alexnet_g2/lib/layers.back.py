...
class ConvPoolLayer(object):
    ...
    def __init__(self, input, image_shape, filter_shape, convstride, padsize,
        ...
        if self.lrn:
            ...
            normOp = norm_lrn_mkldnn.NormAcrossMap(alpha=1e-4, beta=0.75, n=5, k=1, uniq_id=layer_idx)
        ...
        if group == 1:
            ...
            conv_out = conv_mkldnn.conv_forward(subsample=(convstride, convstride),
                                                ...
                                                kshp=self.filter_shape)(input, self.W.val, self.b.val)
        else:
            conv_out0 = \
        ...
        #reluOp = relu_mkldnn.Relu(uniq_id=1)

        self.output = conv_out

        reluOp = relu_mkldnn.Relu(uniq_id=layer_idx)
...
#                                        ignore_border=True)
            poolOp = pool_mkldnn.Pool(ds=(poolsize, poolsize), st=(poolstride, poolstride), ignore_border=True, uniq_id=layer_idx)
